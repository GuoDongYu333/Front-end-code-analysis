<template>
  <wp-modal
    class="mp-add-variations-dialog"
    :visible="addDialogVisible"
    :title="$kh('nominateSelectVariations')"
    :width="846"
    @cancel="closeDialog"
  >
    <section slot="content" class="add-variations-body">
      <wp-tabs v-model="activeTab">
        <wp-tab-pane :label="$kh('nominateTabSelect')" name="select">
          <div class="search-select">
            <div class="search-form">
              <wp-cascader
                :options="kindTree"
                :popper-class="`mp-kind-cascader`"
                v-model="searchForm.kind_ids"
                @change="changekind"
                :clearable="canClearkind"
              />
              <wp-input-group :prepend-width="160">
                <wp-select
                  slot="prepend"
                  v-model="searchForm.field"
                  :placeholder="$kh('commonSelectPlaceholder')"
                >
                  <wp-option
                    v-for="item in productOptions"
                    :key="item.field"
                    :value="item.field"
                    >{{ item.label }}</wp-option
                  >
                </wp-select>
                <wp-input
                  clearable
            ...
            <div class="status-filter">
              <wp-checkbox
                ...
                v-model="searchForm.available_only"
                >{{ $kh("nominateShowAvailable") }}</wp-checkbox
              >
              <wp-popover :content="availableOnlyTip" trigger="hover">
              </wp-popover>
            </div>
            <wp-table
              class="add-variation-table"
              :row-class-name="getRowClassName"
              :data="currentList"
              :height="300"
              :loading="loading"
              :empty-text="$kh('commonNoVariation')"
              valign="middle"
            >
              <wp-table-column label=" " :width="40">
                <template slot-scope="props">
                  <span
                    v-if="props.row.kyList && props.row.kyList.length"
                    class="expand-cell"
                    @click="switchItemExpand(props.row)"
                  >
                  </span>
                </template>
              </wp-table-column>
              <wp-table-column :width="32">
                ...
                <template slot-scope="props">
                  <wp-checkbox
                    @change="
                      selectRows(
                        [props.row, ...(props.row.kyList || [])],
                        !getCheckValue(props.row)
                      )
                    "
                    :acturalValue="getCheckValue(props.row)"
        ...
        <wp-tab-pane :label="$kh('nominateBatchUpload')" name="batch">
          <batch-upload
            @addVariations="addBatchVariations"
